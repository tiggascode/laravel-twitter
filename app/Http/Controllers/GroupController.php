...
class GroupController extends Controller
{
    ...
    public function updateImage(Request $request, Group $group)
    {
        if (!$group->isAdmin(Auth::id())) {
            return response('Your don\'t have prermission to perform this action', 403);
        }
    ...
    public function approveInvitation(string $token)
    {
        ...
        } else if ($groupUser->toke_used || $groupUser->status === GroupUserStatus::APPROVED->value) {
    ...
    {
        if (!$group->isAdmin(Auth::id())) {
            return response('You dont have permission to perform this action', 403);
        }
    ...
    public function changeRole(Request $request, Group $group)
    {
        ...
        if ($user_id == $group->isOwner($user_id)) {
            return response('You cant change role of the owner of the group', 403);
        }
        $groupUser = GroupUser::where('user_id', $user_id)
            ->where('group_id', $group->id)
            ->first();
